import type { Lang } from "./SiteHeader";

const OFFICIAL = {
  ssf: "https://ssf.gob.sv/servicios/tasas-de-interes-comisiones-y-recargos/",
  mtps: "https://www.mtps.gob.sv/servicios/calculo-de-indemnizacion-en-linea/",
...
const copy = {
  es: {
    ...
    tools: "Herramientas", loans: "Préstamos", settlement: "Finiquito e indemnización", withholding: "Retenciones salariales",
  ...
  en: {
    ...
    tools: "Tools", loans: "Loans", settlement: "Settlement and severance", withholding: "Payroll withholding",
  ...
  return <footer className="site-footer">
    <div className="footer-main">
      <div className="footer-intro">
        <a className="brand footer-brand" href="/" aria-label="LoanPilot"><span>LP</span> LoanPilot</a>
        <p>{t.statement}</p>
      ...
      <nav className="footer-column" aria-label={t.tools}>
        <h2>{t.tools}</h2>
        <a href="/prestamos/">{t.loans}<span>→</span></a>
        <a href="/finiquito/">{t.settlement}<span>→</span></a>
        <a href="/retenciones/">{t.withholding}<span>→</span></a>
      </nav>
      <nav className="footer-column" aria-label={t.sources}>
        <h2>{t.sources}</h2>
        <a href={OFFICIAL.ssf} target="_blank" rel="noreferrer">{t.ssf}<span>↗</span></a>
        <a href={OFFICIAL.mtps} target="_blank" rel="noreferrer">{t.mtps}<span>↗</span></a>
